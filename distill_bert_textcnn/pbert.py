...
class Processor(object):
    def get_train_examples(self, data_dir):
        ...
        :return:
        """
        return self._create_examples(os.path.join(data_dir, 'train_data.txt'), 'train')
    ...
    def get_test_examples(self, data_dir):
        ...
        :return:
        """
        return self._create_examples(os.path.join(data_dir, 'test_data.txt'), 'test')
    ...
    def get_dev_examples(self, data_dir):
        ...
        :return:
        """
        return self._create_examples(os.path.join(data_dir, 'dev_data.txt'), 'dev')
    ...
    def _create_examples(self, data_path, set_type):
        ...
        with open(data_path, encoding="utf-8") as f:
            for i, line in enumerate(f):
                ...
                guid = "{0}-{1}-{2}".format(set_type, templabel, i)
                examples.append(InputExample(guid=guid, text=text, label=templabel))
...
class BertClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, input_mask, label_ids):
        ...
        :return:
        """
        _, pooled_output = self.bert(input_ids, None, input_mask, return_dict=False)
...
def main(bert_model='bert-base-chinese',
    ...
    # 读取训练数据集
    processor = Processor()
    train_examples = processor.get_train_examples('../data/ad')
    ...
    # eval_examples = processor.get_dev_examples('data/ad')
    eval_examples = processor.get_test_examples('../data/ad')
